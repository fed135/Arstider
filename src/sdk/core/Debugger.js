...
define( "Arstider/core/Debugger", ["Arstider/core/Performance","Arstider/Keyboard","Arstider/core/Profiler", "Arstider/Events", "Arstider/Viewport"], function /** @lends Debugger */ (Performance, Keyboard, Profiler, Events, Viewport){
	...
	function gradeColor(min, max, current, reverse){
		...
		if(reverse){
			red = Arstider.chop(((current - min)/(max-min)) * 255);
			green = 255 - Arstider.chop(((current - min)/(max-min)) * 255);
		}
		else{
			red = 255 - Arstider.chop(((current - min)/(max-min)) * 255);
			green = Arstider.chop(((current - min)/(max-min)) * 255);
		}
		
		inc = Arstider.floor(Arstider.max(red-(255-red), 0)*0.6 + Arstider.max(green-(255-green))*0.6);
	...
	Debugger.prototype.step = function(){
		...
		if(!this.profiler.collapsed){
			...
			if(this.profiler.currentTab === "draw"){
				...
				if(Performance.frames > 0){
					...
					var numEntities = Arstider.ceil((Performance.elements - Performance.draws)/Performance.frames);
					var numDraws = Arstider.ceil(Performance.draws/Performance.frames);
					var numTransforms = Arstider.ceil(Performance.transforms/Performance.frames);
					var numComposition = Arstider.ceil((Performance.composition || 0)/Performance.frames);
					
					var total = numEntities + numDraws + numTransforms + numComposition;
					var currPos = 0;
					var maxHeight = 300;
					
					var entitiesSection = this.createSection(numEntities + " empty element(s)", 0, Arstider.chop((numEntities/total)*maxHeight), "rgba(150,150,190,1)");
					currPos += Arstider.chop((numEntities/total)*maxHeight);
					newCol.appendChild(entitiesSection);
					
					var drawsSection = this.createSection(numDraws + " draw(s)", currPos, Arstider.chop((numDraws/total)*maxHeight), "rgba(200,200,50,1)");
					currPos += Arstider.chop((numDraws/total)*maxHeight);
					newCol.appendChild(drawsSection);
					
					var transformSection = this.createSection(numTransforms + " transformation(s)", currPos, Arstider.chop((numTransforms/total)*maxHeight), "#f80");
					currPos += Arstider.chop((numTransforms/total)*maxHeight);
					newCol.appendChild(transformSection);
					
					
					var compSection = this.createSection(numComposition + " composion mode change(s)", currPos, Arstider.chop((numComposition/total)*maxHeight), "#f03");
					currPos += Arstider.chop((numComposition/total)*maxHeight);
					...
					fpsBar.style.top = (300 - Arstider.chop((Performance.frames/65)*150)) + "px";
		...
		this.profiler.minibar.innerHTML = [
			                           ...
			                           "&nbsp;|&nbsp;",
			                           //Drawn Objects
			                           "<span style='color:#FF9999;'>Draws:", Arstider.ceil(Performance.draws/Performance.frames) ,"/ Entities:",Arstider.ceil(Performance.elements/Performance.frames),"</span>",
			                           "&nbsp;|&nbsp;",
			                           //Transformations
			                           "<span style='color:#9999FF;'>Transformations:",Arstider.ceil(Performance.transforms/Performance.frames),"</span>",
			                           "&nbsp;|&nbsp;",
			                           //Num Buffers
